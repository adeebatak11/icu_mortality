...
# GLM AUC 95% CI
boot_glm_auc <- boot(data = df_model, statistic = glm_boot_auc, R = 200)
...
# LASSO AUC 95% CI
boot_lasso_auc <- boot(data = df_model, statistic = lasso_boot_auc, R = 200)
...
ggplot(auc_plot_df, aes(x = Model, y = MeanAUC)) +
  ...
  geom_text(aes(label = sprintf("%.3f", MeanAUC)),
            position = position_nudge(x = 0.07), 
            size = 5, fontface = "bold", hjust = 0) +
  labs(
    title = "Model Discrimination (AUC) with Trimmed 90% CIs",
    subtitle = "Bootstrap resampling (n = 1,000)",
